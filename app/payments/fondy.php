...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if ($mode == 'response' && !empty($_POST['order_id'])) {
        if ($response === true && $order_info['status'] != 'P') {
            if ($_POST['order_status'] == FondyCls::ORDER_APPROVED) {
    ...
    } elseif ($mode == 'sucsses' && !empty($_POST['order_id'])) {
        if ($response === true && $order_info['status'] != 'P') {
            if ($_POST['order_status'] == FondyCls::ORDER_APPROVED) {
                $pp_response['order_status'] = 'P';
                $pp_response['reason_text'] = __('transaction_approved');
            ...
            } elseif ($_POST['order_status'] == FondyCls::ORDER_DECLINED) {
                $pp_response['transaction_id'] = $_POST['payment_id'];
                if (fn_check_payment_script('fondy.php', $order_id)) {
                    fn_finish_payment($order_id, $pp_response);
                    exit('Ok');
